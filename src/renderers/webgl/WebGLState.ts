...
class WebGLState {
    ...
    currentBoundTextures: {
        [index: number]: { type: number; texture: GLESTexture };
    } = {};
    ...
    constructor(
        ...
        if (this.glVersion.indexOf('WebGL') !== -1) {
            ...
            this.version = parseFloat(
                (/^OpenGL ES (\d)/).exec(this.glVersion)[1]
            );
    ...
    compressedTexImage2D(...args: Parameters<GLESRenderingContext['compressedTexImage2D']>) {
        try {
            this._gl.compressedTexImage2D(...args);
        } catch (error) {
            console.error('WebGLState:', error);
        }
    }

    texImage2D(...args: Parameters<GLESRenderingContext['texImage2D']>) {
        try {
            this._gl.texImage2D(...args);
        } catch (error) {
            console.error('WebGLState:', error);
        }
    }

    texImage3D(...args: Parameters<GLESRenderingContext['texImage3D']>) {
        try {
            this._gl.texImage3D(...args);
        } catch (error) {
            console.error('WebGLState:', error);
        }
    }
