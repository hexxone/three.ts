...
class WebGLPrograms {
    ...
    getParameters(
        ...
        if (shadr) {
            ({ vertexShader } = shadr);
            ({ fragmentShader } = shadr);
        } else {
            ({ vertexShader } = shaderMaterial);
            ({ fragmentShader } = shaderMaterial);
        }
        ...
        return {
            ...
            vertexTangents:
                material.normalMap && meshStdMaterial.vertexTangents,
            ...
            shadowMapEnabled:
                this._renderer.shadowMap.enabled && shadows.length > 0,
