            ...
            for (let i = 0, l = json.length; i < l; i++) {
                ...
                switch (data.type) {
                    ...
                    case 'ExtrudeBufferGeometry':
                        ...
                        const { extrudePath } = data.options;
        ...
        if (json !== undefined) {
            for (let i = 0, l = json.length; i < l; i++) {
                ...
                if (data.image === undefined) {
                    console.warn(
                        'ObjectLoader: No "image" specified for',
                        data.uuid
                    );
                }
    ...
    parseObject(data, geometries?, materials?, animations?) {
        ...
        switch (data.type) {
            ...
            case 'PerspectiveCamera':
                const pc = (object = new PerspectiveCamera(
                    data.fov,
                    data.aspect,
                    data.near,
                    data.far
                ...
                if (data.filmGauge !== undefined) {
                    pc.filmGauge = data.filmGauge;
                }
                if (data.filmOffset !== undefined) {
                    pc.filmOffset = data.filmOffset;
                }
            ...
            case 'OrthographicCamera':
                const oc = (object = new OrthographicCamera(
                    data.left,
                    data.right,
                    data.top,
                    data.bottom,
        ...
        if (data.shadow) {
            const li = object as Light;

            if (data.shadow.bias !== undefined) {
                li.shadow.bias = data.shadow.bias;
            }
            ...
            if (data.shadow.camera !== undefined) {
                li.shadow.camera = this.parseObject(
                    data.shadow.camera
                ) as Camera;
            }
        }
