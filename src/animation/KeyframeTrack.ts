import { InterpolateDiscrete,
    InterpolateLinear,
    InterpolateSmooth } from '../constants';
...
class KeyframeTrack {
    ...
    constructor(
        ...
        if (times === undefined || times.length === 0) {
            throw new Error(
                `KeyframeTrack: no keyframes in track named ${name}`
            );
        }
    ...
    optimize() {
        ...
        const smoothInterpolation
            = this.getInterpolation() === InterpolateSmooth;
        ...
        if (lastIndex > 0) {
            ...
            for (
                ...
                values[writeOffset + x] = values[readOffset + x];
            }
    ...
    clone() {
        ...
        const track = new KeyframeTrack(this.name, times, values);
