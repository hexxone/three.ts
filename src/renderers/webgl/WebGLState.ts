import { LessEqualDepth, CullFaceFront, CullFaceBack, CullFaceNone, DoubleSide, BackSide, CustomBlending, MultiplyBlending, SubtractiveBlending, AdditiveBlending, NoBlending, NormalBlending, AddEquation, SubtractEquation, ReverseSubtractEquation, MinEquation, MaxEquation, ZeroFactor, OneFactor, SrcColorFactor, SrcAlphaFactor, SrcAlphaSaturateFactor, DstColorFactor, DstAlphaFactor, OneMinusSrcColorFactor, OneMinusSrcAlphaFactor, OneMinusDstColorFactor, OneMinusDstAlphaFactor, Material, Vector4 } from '../../';
...
class WebGLState {
	...
	constructor( gl: GLESRenderingContext, extensions: WebGLExtensions, capabilities: WebGLCapabilities ) {
		...
		this.emptyTextures[ gl.TEXTURE_2D ] = this.createTexture( this._gl.TEXTURE_2D, this._gl.TEXTURE_2D, 1 );
		this.emptyTextures[ gl.TEXTURE_CUBE_MAP ] = this.createTexture( this._gl.TEXTURE_CUBE_MAP, this._gl.TEXTURE_CUBE_MAP_POSITIVE_X, 6 );
	...
	createTexture( type: number, target: number, count: number ) {
		...
		for ( let i = 0; i < count; i ++ ) {
			this._gl.texImage2D( target + i, 0, this._gl.RGBA, 1, 1, 0, this._gl.RGBA, this._gl.UNSIGNED_BYTE, data );
		}
	...
	setBlending( blending: number, blendEquation?: number, blendSrc?: number, blendDst?: number, blendEquationAlpha?: number, blendSrcAlpha?: number, blendDstAlpha?: number, premultipliedAlpha?: boolean ) {
			...
			if ( blending !== this.currentBlending || premultipliedAlpha !== this.currentPremultipledAlpha ) {
				...
				if ( premultipliedAlpha ) {
					...
					case SubtractiveBlending:
						this._gl.blendFuncSeparate( this._gl.ZERO, this._gl.ZERO, this._gl.ONE_MINUS_SRC_COLOR, this._gl.ONE_MINUS_SRC_ALPHA );
						break;

					case MultiplyBlending:
						this._gl.blendFuncSeparate( this._gl.ZERO, this._gl.SRC_COLOR, this._gl.ZERO, this._gl.SRC_ALPHA );
						break;
		...
		if ( blendEquation !== this.currentBlendEquation || blendEquationAlpha !== this.currentBlendEquationAlpha ) {
			this._gl.blendEquationSeparate( this.equationToGL[ blendEquation ], this.equationToGL[ blendEquationAlpha ] );
		...
		if ( blendSrc !== this.currentBlendSrc || blendDst !== this.currentBlendDst || blendSrcAlpha !== this.currentBlendSrcAlpha || blendDstAlpha !== this.currentBlendDstAlpha ) {
			this._gl.blendFuncSeparate( this.factorToGL[ blendSrc ], this.factorToGL[ blendDst ], this.factorToGL[ blendSrcAlpha ], this.factorToGL[ blendDstAlpha ] );
	...
	activeTexture( webglSlot?: number ) {
		if ( webglSlot === undefined ) webglSlot = this._gl.TEXTURE0 + this.maxTextures - 1;
	...
	bindTexture( webglType: number, webglTexture: GLESTexture ) {
		...
		if ( boundTexture.type !== webglType || boundTexture.texture !== webglTexture ) {
			this._gl.bindTexture( webglType, webglTexture || this.emptyTextures[ webglType ] );
