...
class WebGLTextures {
	...
	setTexture2D( texture, slot ) {
		...
		this._state.bindTexture( this._gl.TEXTURE_2D, textureProperties.__webglTexture );
	}
	...
	setTexture2DArray( texture, slot ) {
		...
		this._state.activeTexture( this._gl.TEXTURE0 + slot );
		this._state.bindTexture( this._gl.TEXTURE_2D_ARRAY, textureProperties.__webglTexture );
	}
	...
	setTexture3D( texture, slot ) {
		...
		this._state.activeTexture( this._gl.TEXTURE0 + slot );
		this._state.bindTexture( this._gl.TEXTURE_3D, textureProperties.__webglTexture );
	}
	...
	setTextureCube( texture, slot ) {
		...
		this._state.activeTexture( this._gl.TEXTURE0 + slot );
		this._state.bindTexture( this._gl.TEXTURE_CUBE_MAP, textureProperties.__webglTexture );
	}
	...
	setTextureParameters( textureType, texture, supportsMips ) {
		...
		if ( this._extensions.has( 'EXT_texture_filter_anisotropic' ) === true ) {
			...
			if ( texture.type === FloatType && this._extensions.has( 'OES_texture_float_linear' ) === false ) return; // verify extension for WebGL 1 and WebGL 2
			if ( this.isWebGL2 === false && ( texture.type === HalfFloatType && this._extensions.has( 'OES_texture_half_float_linear' ) === false ) ) return; // verify extension for WebGL 1 only

			if ( texture.anisotropy > 1 || this._properties.get( texture ).__currentAnisotropy ) {
				this._gl.texParameterf( textureType, extension.TEXTURE_MAX_ANISOTROPY_EXT, Math.min( texture.anisotropy, this._capabilities.getMaxAnisotropy() ) );
	...
	uploadTexture( textureProperties, texture, slot ) {
		...
		if ( texture.isDepthTexture ) {
			...
			this._state.texImage2D( this._gl.TEXTURE_2D, 0, glInternalFormat, image.width, image.height, 0, glFormat, glType, null );
			...
			if ( mipmaps.length > 0 && supportsMips ) {
				for ( let i = 0, il = mipmaps.length; i < il; i ++ ) {
					mipmap = mipmaps[ i ];
					this._state.texImage2D( this._gl.TEXTURE_2D, i, glInternalFormat, mipmap.width, mipmap.height, 0, glFormat, glType, mipmap.data );
				}

				texture.generateMipmaps = false;
				textureProperties.__maxMipLevel = mipmaps.length - 1;
			} else {
				this._state.texImage2D( this._gl.TEXTURE_2D, 0, glInternalFormat, image.width, image.height, 0, glFormat, glType, image.data );
			...
			for ( let i = 0, il = mipmaps.length; i < il; i ++ ) {
				...
				if ( texture.format !== RGBAFormat && texture.format !== RGBFormat ) {
					if ( glFormat !== null ) {
						this._state.compressedTexImage2D( this._gl.TEXTURE_2D, i, glInternalFormat, mipmap.width, mipmap.height, 0, mipmap.data );
					} else {
			...
			this._state.texImage3D( this._gl.TEXTURE_2D_ARRAY, 0, glInternalFormat, image.width, image.height, image.depth, 0, glFormat, glType, image.data );
			textureProperties.__maxMipLevel = 0;
		} else if ( texture.isDataTexture3D ) {
			this._state.texImage3D( this._gl.TEXTURE_3D, 0, glInternalFormat, image.width, image.height, image.depth, 0, glFormat, glType, image.data );
			textureProperties.__maxMipLevel = 0;
		} else {
			...
			if ( mipmaps.length > 0 && supportsMips ) {
				for ( let i = 0, il = mipmaps.length; i < il; i ++ ) {
					mipmap = mipmaps[ i ];
					this._state.texImage2D( this._gl.TEXTURE_2D, i, glInternalFormat, glFormat, glType, mipmap );
				}

				texture.generateMipmaps = false;
				textureProperties.__maxMipLevel = mipmaps.length - 1;
			} else {
				this._state.texImage2D( this._gl.TEXTURE_2D, 0, glInternalFormat, glFormat, glType, image );
				textureProperties.__maxMipLevel = 0;
			}
		}
		...
		if ( isCompressed ) {
			for ( let i = 0; i < 6; i ++ ) {
				...
				for ( let j = 0; j < mipmaps.length; j ++ ) {
					...
					if ( texture.format !== RGBAFormat && texture.format !== RGBFormat ) {
						if ( glFormat !== null ) {
							this._state.compressedTexImage2D( this._gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, j, glInternalFormat, mipmap.width, mipmap.height, 0, mipmap.data );
						} else {
							console.warn( 'THREE.WebGLRenderer: Attempt to load unsupported compressed texture format in .setTextureCube()' );
						}
					} else {
						this._state.texImage2D( this._gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, j, glInternalFormat, mipmap.width, mipmap.height, 0, glFormat, glType, mipmap.data );
					}
				}
			}
			...
			for ( let i = 0; i < 6; i ++ ) {
				if ( isDataTexture ) {
					this._state.texImage2D( this._gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, cubeImage[ i ].width, cubeImage[ i ].height, 0, glFormat, glType, cubeImage[ i ].data );

					for ( let j = 0; j < mipmaps.length; j ++ ) {
						const mipmap = mipmaps[ j ];
						const mipmapImage = mipmap.image[ i ].image;

						this._state.texImage2D( this._gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, j + 1, glInternalFormat, mipmapImage.width, mipmapImage.height, 0, glFormat, glType, mipmapImage.data );
					}
				} else {
					this._state.texImage2D( this._gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, glInternalFormat, glFormat, glType, cubeImage[ i ] );

					for ( let j = 0; j < mipmaps.length; j ++ ) {
						const mipmap = mipmaps[ j ];

						this._state.texImage2D( this._gl.TEXTURE_CUBE_MAP_POSITIVE_X + i, j + 1, glInternalFormat, glFormat, glType, mipmap.image[ i ] );
					}
				}
			}
	...
	setupFrameBufferTexture( framebuffer, renderTarget, attachment, textureTarget ) {
		...
		if ( textureTarget === this._gl.TEXTURE_3D || textureTarget === this._gl.TEXTURE_2D_ARRAY ) {
			this._state.texImage3D( textureTarget, 0, glInternalFormat, renderTarget.width, renderTarget.height, renderTarget.depth, 0, glFormat, glType, null );
		} else {
			this._state.texImage2D( textureTarget, 0, glInternalFormat, renderTarget.width, renderTarget.height, 0, glFormat, glType, null );
		}

		this._gl.bindFramebuffer( this._gl.FRAMEBUFFER, framebuffer );
		this._gl.framebufferTexture2D( this._gl.FRAMEBUFFER, attachment, textureTarget, this._properties.get( texture ).__webglTexture, 0 );
	...
	setupRenderTarget( renderTarget ) {
		...
		if ( isCube ) {
			...
			this._state.bindTexture( glTextureType, textureProperties.__webglTexture );
