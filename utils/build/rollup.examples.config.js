...
function unmodularize() {
    return {
        renderChunk(code, { fileName }) {
            ...
            const fileNameNoExtension = fileName.slice(
                0,
                fileName.indexOf('.')
            );
            const namespace = fileNameNoExtension.endsWith('Utils')
                ? fileNameNoExtension
                : undefined;
            ...
            code = code.replace(
                ...
                (match, p1) => {
                    ...
                    if (namespace) {
                        ...
                        output += exps
                            .map((exp) => `THREE.${namespace}.${exp} = ${exp};`)
                            .join(EOL);
                    } else {
                        output += exps
                            .map((exp) => `THREE.${exp} = ${exp};`)
                            .join(EOL);
                    }
            ...
            // but excluding imports importing from the libs/ folder
            code = code.replace(
                /import \* as ([a-zA-Z0-9_, ]+) from '((?!libs).)*';/g,
                (match, p1) => {
                    const imp = p1;
                    if (imp !== 'THREE') {
                        imports.push(imp);
            ...
            function prefixThree(word) {
                code = code.replace(
                    new RegExp(`([\\s([!])${word}([^a-zA-Z0-9_])`, 'g'),
                    (match, p1, p2) => {
