...
const materialsContent = fs.readFileSync(materialsFilePath, {
    encoding: 'utf8'
});
...
function parseObject(fileName, isRoot) {
    ...
    const objectPath = path.join(prefix, fileName).trim()
        .replace(/\\/g, '/');
